...
// Source: common/types/context_update_request.json (spec release/2026-08-25@cd78fb3)
...
type ContextUpdateRequest struct {
	...
	Payment []ContextUpdateRequestPaymentItem `json:"payment,omitzero"`
...
// ContextUpdateRequestPaymentItem is generated from common/types/context_update_request.json.
type ContextUpdateRequestPaymentItem struct {
	// Handler registry key advertised in the Business profile's `ucp.payment_handlers`.
	Handler ReverseDomainNameUpdateRequest `json:"handler"`
...
// UnmarshalJSON decodes the named properties and keeps everything else
// in Extra.
func (v *ContextUpdateRequestPaymentItem) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("ContextUpdateRequestPaymentItem: null is not a valid object")
	}
	type ContextUpdateRequestPaymentItemAlias ContextUpdateRequestPaymentItem
	var named ContextUpdateRequestPaymentItemAlias
	if err := json.Unmarshal(data, &named); err != nil {
		return err
	}
	*v = ContextUpdateRequestPaymentItem(named)
...
// MarshalJSON emits the named properties alongside anything held in
// Extra.
func (v ContextUpdateRequestPaymentItem) MarshalJSON() ([]byte, error) {
	type ContextUpdateRequestPaymentItemAlias ContextUpdateRequestPaymentItem
	named, err := json.Marshal(ContextUpdateRequestPaymentItemAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
...
// Validate reports the first constraint violation, or nil.
func (v *ContextUpdateRequestPaymentItem) Validate() error {
	if v.present != nil {
