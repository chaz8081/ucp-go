...
// Source: common/types/geo.json (spec release/2026-08-25@cd78fb3)
...
// Geo WGS 84 geographic coordinates in decimal degrees.
type Geo struct {
...
func (v *Geo) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("Geo: null is not a valid object")
	}
	type GeoAlias Geo
	var named GeoAlias
...
func (v Geo) MarshalJSON() ([]byte, error) {
	type GeoAlias Geo
	named, err := json.Marshal(GeoAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
