...
// Source: common/types/measure.json (spec release/2026-08-25@cd78fb3)
...
// Measure A measure composed of an integer value and a unit descriptor. Its value is the integer count of `10^-scale` units of `unit`.
type Measure struct {
...
func (v *Measure) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("Measure: null is not a valid object")
	}
	type MeasureAlias Measure
	var named MeasureAlias
...
func (v Measure) MarshalJSON() ([]byte, error) {
	type MeasureAlias Measure
	named, err := json.Marshal(MeasureAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
