...
// Source: common/types/measure_update_request.json (spec release/2026-08-25@cd78fb3)
...
// MeasureUpdateRequest A measure composed of an integer value and a unit descriptor. Its value is the integer count of `10^-scale` units of `unit`.
...
// in Extra.
func (v *MeasureUpdateRequest) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("MeasureUpdateRequest: null is not a valid object")
	}
	type MeasureUpdateRequestAlias MeasureUpdateRequest
	var named MeasureUpdateRequestAlias
	if err := json.Unmarshal(data, &named); err != nil {
		return err
	}
	*v = MeasureUpdateRequest(named)
...
// Extra.
func (v MeasureUpdateRequest) MarshalJSON() ([]byte, error) {
	type MeasureUpdateRequestAlias MeasureUpdateRequest
	named, err := json.Marshal(MeasureUpdateRequestAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
...
func (v *MeasureUpdateRequest) Validate() error {
	if v.present != nil {
