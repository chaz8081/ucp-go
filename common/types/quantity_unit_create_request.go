...
// Source: common/types/quantity_unit_create_request.json (spec release/2026-08-25@cd78fb3)
...
// QuantityUnitCreateRequest Sale-basis descriptor for quantities: the shared unit descriptor plus the Business's ordering policy. Its unit-descriptor machine identity remains (`unit`, effective `scale`); `display_text` and `increment` are excluded from identity and mismatch comparison.
type QuantityUnitCreateRequest struct {
...
// in Extra.
func (v *QuantityUnitCreateRequest) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("QuantityUnitCreateRequest: null is not a valid object")
	}
	type QuantityUnitCreateRequestAlias QuantityUnitCreateRequest
	var named QuantityUnitCreateRequestAlias
	if err := json.Unmarshal(data, &named); err != nil {
		return err
	}
	*v = QuantityUnitCreateRequest(named)
...
// Extra.
func (v QuantityUnitCreateRequest) MarshalJSON() ([]byte, error) {
	type QuantityUnitCreateRequestAlias QuantityUnitCreateRequest
	named, err := json.Marshal(QuantityUnitCreateRequestAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
...
func (v *QuantityUnitCreateRequest) Validate() error {
	if v.present != nil {
