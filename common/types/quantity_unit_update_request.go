...
// Source: common/types/quantity_unit_update_request.json (spec release/2026-08-25@cd78fb3)
...
// QuantityUnitUpdateRequest Sale-basis descriptor for quantities: the shared unit descriptor plus the Business's ordering policy. Its unit-descriptor machine identity remains (`unit`, effective `scale`); `display_text` and `increment` are excluded from identity and mismatch comparison.
type QuantityUnitUpdateRequest struct {
...
// in Extra.
func (v *QuantityUnitUpdateRequest) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("QuantityUnitUpdateRequest: null is not a valid object")
	}
	type QuantityUnitUpdateRequestAlias QuantityUnitUpdateRequest
	var named QuantityUnitUpdateRequestAlias
	if err := json.Unmarshal(data, &named); err != nil {
		return err
	}
	*v = QuantityUnitUpdateRequest(named)
...
// Extra.
func (v QuantityUnitUpdateRequest) MarshalJSON() ([]byte, error) {
	type QuantityUnitUpdateRequestAlias QuantityUnitUpdateRequest
	named, err := json.Marshal(QuantityUnitUpdateRequestAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
...
func (v *QuantityUnitUpdateRequest) Validate() error {
	if v.present != nil {
