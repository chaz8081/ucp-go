...
// Source: common/types/unit.json (spec release/2026-08-25@cd78fb3)
...
// Unit A reusable unit descriptor for quantities and measures. Its unit-descriptor machine identity is (`unit`, effective `scale`), where effective `scale` is the provided `scale` or 0; `display_text` is excluded.
type Unit struct {
...
func (v *Unit) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("Unit: null is not a valid object")
	}
	type UnitAlias Unit
	var named UnitAlias
...
func (v Unit) MarshalJSON() ([]byte, error) {
	type UnitAlias Unit
	named, err := json.Marshal(UnitAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
