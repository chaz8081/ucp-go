...
// Source: common/types/unit_create_request.json (spec release/2026-08-25@cd78fb3)
...
// UnitCreateRequest A reusable unit descriptor for quantities and measures. Its unit-descriptor machine identity is (`unit`, effective `scale`), where effective `scale` is the provided `scale` or 0; `display_text` is excluded.
...
// in Extra.
func (v *UnitCreateRequest) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("UnitCreateRequest: null is not a valid object")
	}
	type UnitCreateRequestAlias UnitCreateRequest
	var named UnitCreateRequestAlias
	if err := json.Unmarshal(data, &named); err != nil {
		return err
	}
	*v = UnitCreateRequest(named)
...
// Extra.
func (v UnitCreateRequest) MarshalJSON() ([]byte, error) {
	type UnitCreateRequestAlias UnitCreateRequest
	named, err := json.Marshal(UnitCreateRequestAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
...
func (v *UnitCreateRequest) Validate() error {
	if v.present != nil {
