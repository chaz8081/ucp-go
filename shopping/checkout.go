...
// Source: shopping/checkout.json (spec release/2026-08-25@cd78fb3)
...
// Checkout Base checkout schema. Extensions compose onto this using allOf.
...
func (v *Checkout) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("Checkout: null is not a valid object")
	}
	type CheckoutAlias Checkout
	var named CheckoutAlias
...
func (v Checkout) MarshalJSON() ([]byte, error) {
	type CheckoutAlias Checkout
	named, err := json.Marshal(CheckoutAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
...
func (v *Checkout) Validate() error {
	if v.present != nil {
