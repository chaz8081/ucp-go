...
// Source: shopping/types/order_line_item.json (spec release/2026-08-25@cd78fb3)
...
type OrderLineItem struct {
	...
	Totals []commontypes.Total `json:"totals"`
...
// OrderLineItemQuantity Tracks the line item's original, current active, and fulfilled quantities. All three values use the same inherited `item.quantity_unit`. When `item.quantity_unit` is absent on an authoritative order response, each step is one whole item (`each`) under the shared default.
