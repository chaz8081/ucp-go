// Code generated by ucpgen. DO NOT EDIT.
// Source: shopping/types/unit_price.json (spec release/2026-08-25@cd78fb3)

package types

import (
	"encoding/json"
	"errors"
	commontypes "github.com/chaz8081/ucp-go/common/types"
	"regexp"
	"sync"
)

// UnitPrice Price per standard unit of measurement. MAY be omitted when unit pricing does not apply. `unit_price.currency` MUST equal `price.currency`; the comparator MUST NOT perform currency conversion. `measure.unit` and `reference.unit` MUST be identical; cross-unit conversion is not permitted. Their scales MAY differ; each value represents `value × 10^-scale`.
type UnitPrice struct {
	// Unit price in ISO 4217 minor units. After satisfying the same-unit invariant, the Business MUST compute the comparator as `(price.amount / (measure.value × 10^-measure.scale)) × (reference.value × 10^-reference.scale)` and round it once to ISO 4217 minor units according to its pricing rules. The returned `unit_price.amount` is authoritative; the Platform MUST NOT recompute or substitute its own result.
	Amount commontypes.Amount `json:"amount"`
	// ISO 4217 currency code.
	Currency string `json:"currency"`
	// Product quantity in packaging/content (for example, a 750 mL bottle), distinct from `quantity_unit`, which defines the sale basis. Its integer `value` MUST be at least 1.
	//
	// Not enforced yet: if, then.
	Measure UnitPriceMeasure `json:"measure"`
	// Denominator for unit price display (for example, per 100 mL or per 1 kg). Its integer `value` MUST be at least 1.
	//
	// Not enforced yet: if, then.
	Reference UnitPriceReference `json:"reference"`

	// Extra holds properties the schema does not name. The schema is
	// open (additionalProperties is not false), so extension keys are
	// preserved here and re-emitted on marshal rather than dropped.
	Extra map[string]json.RawMessage `json:"-"`

	// present records which properties the decoder saw, so a required
	// property that was absent can be told from one decoded to its zero
	// value. A nil map means this value was never decoded from JSON.
	present map[string]bool
}

// UnmarshalJSON decodes the named properties and keeps everything else
// in Extra.
func (v *UnitPrice) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("UnitPrice: null is not a valid object")
	}
	type UnitPriceAlias UnitPrice
	var named UnitPriceAlias
	if err := json.Unmarshal(data, &named); err != nil {
		return err
	}
	*v = UnitPrice(named)

	var all map[string]json.RawMessage
	if err := json.Unmarshal(data, &all); err != nil {
		return err
	}
	v.present = make(map[string]bool, 4)
	for _, name := range []string{"amount", "currency", "measure", "reference"} {
		if _, ok := all[name]; ok {
			v.present[name] = true
		}
	}
	delete(all, "amount")
	delete(all, "currency")
	delete(all, "measure")
	delete(all, "reference")
	if len(all) > 0 {
		v.Extra = all
	}
	return nil
}

// MarshalJSON emits the named properties alongside anything held in
// Extra.
func (v UnitPrice) MarshalJSON() ([]byte, error) {
	type UnitPriceAlias UnitPrice
	named, err := json.Marshal(UnitPriceAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
		return named, nil
	}
	var merged map[string]json.RawMessage
	if err := json.Unmarshal(named, &merged); err != nil {
		return nil, err
	}
	if merged == nil {
		merged = map[string]json.RawMessage{}
	}
	for k, val := range v.Extra {
		if _, named := merged[k]; !named {
			merged[k] = val
		}
	}
	return json.Marshal(merged)
}

var pattern_UnitPrice_Currency = sync.OnceValue(func() *regexp.Regexp { return regexp.MustCompile("^[A-Z]{3}$") })

// Validate reports the first constraint violation, or nil.
func (v *UnitPrice) Validate() error {
	if v.present != nil {
		if !v.present["amount"] {
			return errors.New("amount: required property is missing")
		}
		if !v.present["currency"] {
			return errors.New("currency: required property is missing")
		}
		if !v.present["measure"] {
			return errors.New("measure: required property is missing")
		}
		if !v.present["reference"] {
			return errors.New("reference: required property is missing")
		}
	}
	if !pattern_UnitPrice_Currency().MatchString(string(v.Currency)) {
		return errors.New("currency: does not match pattern")
	}
	if err := v.Amount.Validate(); err != nil {
		return err
	}
	if err := v.Measure.Validate(); err != nil {
		return err
	}
	if err := v.Reference.Validate(); err != nil {
		return err
	}
	return nil
}

// UnitPriceMeasure Product quantity in packaging/content (for example, a 750 mL bottle), distinct from `quantity_unit`, which defines the sale basis. Its integer `value` MUST be at least 1.
type UnitPriceMeasure struct {
	// Required printable unit label provided by the Business. The Platform MUST use it when it does not recognize `unit`; for a recognized UN/CEFACT Rec 20 Common Code, the Platform MAY substitute its own localized label. It does not participate in unit identity or mismatch comparison.
	DisplayText string `json:"display_text"`
	// One step equals `10^-scale` of `unit`. When `unit` is `C62`, `scale`, if present, MUST be 0. The maximum of 15 is derived from the interoperable integer range: at scale 16 a single whole unit (10^16 steps) is no longer representable, so larger scales cannot denominate one unit of their own basis. Businesses needing finer granularity use a smaller unit.
	Scale *int64 `json:"scale,omitzero"`
	// Stable machine identifier. The Business SHOULD use the exact UN/CEFACT Rec20 Common Code when one accurately identifies the unit. Otherwise, the Business MAY use a custom unit identifier and MUST use it consistently for the same unit. The Platform MUST treat an unrecognized identifier as opaque.
	Unit string `json:"unit"`
	// Integer count of `10^-scale` units of `unit`.
	Value int64 `json:"value"`

	// Extra holds properties the schema does not name. The schema is
	// open (additionalProperties is not false), so extension keys are
	// preserved here and re-emitted on marshal rather than dropped.
	Extra map[string]json.RawMessage `json:"-"`

	// present records which properties the decoder saw, so a required
	// property that was absent can be told from one decoded to its zero
	// value. A nil map means this value was never decoded from JSON.
	present map[string]bool
}

// UnmarshalJSON decodes the named properties and keeps everything else
// in Extra.
func (v *UnitPriceMeasure) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("UnitPriceMeasure: null is not a valid object")
	}
	type UnitPriceMeasureAlias UnitPriceMeasure
	var named UnitPriceMeasureAlias
	if err := json.Unmarshal(data, &named); err != nil {
		return err
	}
	*v = UnitPriceMeasure(named)

	var all map[string]json.RawMessage
	if err := json.Unmarshal(data, &all); err != nil {
		return err
	}
	v.present = make(map[string]bool, 3)
	for _, name := range []string{"display_text", "unit", "value"} {
		if _, ok := all[name]; ok {
			v.present[name] = true
		}
	}
	delete(all, "display_text")
	delete(all, "scale")
	delete(all, "unit")
	delete(all, "value")
	if len(all) > 0 {
		v.Extra = all
	}
	return nil
}

// MarshalJSON emits the named properties alongside anything held in
// Extra.
func (v UnitPriceMeasure) MarshalJSON() ([]byte, error) {
	type UnitPriceMeasureAlias UnitPriceMeasure
	named, err := json.Marshal(UnitPriceMeasureAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
		return named, nil
	}
	var merged map[string]json.RawMessage
	if err := json.Unmarshal(named, &merged); err != nil {
		return nil, err
	}
	if merged == nil {
		merged = map[string]json.RawMessage{}
	}
	for k, val := range v.Extra {
		if _, named := merged[k]; !named {
			merged[k] = val
		}
	}
	return json.Marshal(merged)
}

// Validate reports the first constraint violation, or nil.
func (v *UnitPriceMeasure) Validate() error {
	if v.present != nil {
		if !v.present["display_text"] {
			return errors.New("display_text: required property is missing")
		}
		if !v.present["unit"] {
			return errors.New("unit: required property is missing")
		}
		if !v.present["value"] {
			return errors.New("value: required property is missing")
		}
	}
	if v.Scale != nil && *v.Scale > 15 {
		return errors.New("scale: above maximum 15")
	}
	if v.Scale != nil && *v.Scale < 0 {
		return errors.New("scale: below minimum 0")
	}
	if v.Value > 9007199254740991 {
		return errors.New("value: above maximum 9007199254740991")
	}
	if v.Value < 1 {
		return errors.New("value: below minimum 1")
	}
	if v.Unit == "C62" {
		if v.Scale != nil && *v.Scale != 0 {
			return errors.New("scale: must be 0")
		}
	}
	return nil
}

// UnitPriceReference Denominator for unit price display (for example, per 100 mL or per 1 kg). Its integer `value` MUST be at least 1.
type UnitPriceReference struct {
	// Required printable unit label provided by the Business. The Platform MUST use it when it does not recognize `unit`; for a recognized UN/CEFACT Rec 20 Common Code, the Platform MAY substitute its own localized label. It does not participate in unit identity or mismatch comparison.
	DisplayText string `json:"display_text"`
	// One step equals `10^-scale` of `unit`. When `unit` is `C62`, `scale`, if present, MUST be 0. The maximum of 15 is derived from the interoperable integer range: at scale 16 a single whole unit (10^16 steps) is no longer representable, so larger scales cannot denominate one unit of their own basis. Businesses needing finer granularity use a smaller unit.
	Scale *int64 `json:"scale,omitzero"`
	// Stable machine identifier. The Business SHOULD use the exact UN/CEFACT Rec20 Common Code when one accurately identifies the unit. Otherwise, the Business MAY use a custom unit identifier and MUST use it consistently for the same unit. The Platform MUST treat an unrecognized identifier as opaque.
	Unit string `json:"unit"`
	// Integer count of `10^-scale` units of `unit`.
	Value int64 `json:"value"`

	// Extra holds properties the schema does not name. The schema is
	// open (additionalProperties is not false), so extension keys are
	// preserved here and re-emitted on marshal rather than dropped.
	Extra map[string]json.RawMessage `json:"-"`

	// present records which properties the decoder saw, so a required
	// property that was absent can be told from one decoded to its zero
	// value. A nil map means this value was never decoded from JSON.
	present map[string]bool
}

// UnmarshalJSON decodes the named properties and keeps everything else
// in Extra.
func (v *UnitPriceReference) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		return errors.New("UnitPriceReference: null is not a valid object")
	}
	type UnitPriceReferenceAlias UnitPriceReference
	var named UnitPriceReferenceAlias
	if err := json.Unmarshal(data, &named); err != nil {
		return err
	}
	*v = UnitPriceReference(named)

	var all map[string]json.RawMessage
	if err := json.Unmarshal(data, &all); err != nil {
		return err
	}
	v.present = make(map[string]bool, 3)
	for _, name := range []string{"display_text", "unit", "value"} {
		if _, ok := all[name]; ok {
			v.present[name] = true
		}
	}
	delete(all, "display_text")
	delete(all, "scale")
	delete(all, "unit")
	delete(all, "value")
	if len(all) > 0 {
		v.Extra = all
	}
	return nil
}

// MarshalJSON emits the named properties alongside anything held in
// Extra.
func (v UnitPriceReference) MarshalJSON() ([]byte, error) {
	type UnitPriceReferenceAlias UnitPriceReference
	named, err := json.Marshal(UnitPriceReferenceAlias(v))
	if err != nil {
		return nil, err
	}
	if len(v.Extra) == 0 {
		return named, nil
	}
	var merged map[string]json.RawMessage
	if err := json.Unmarshal(named, &merged); err != nil {
		return nil, err
	}
	if merged == nil {
		merged = map[string]json.RawMessage{}
	}
	for k, val := range v.Extra {
		if _, named := merged[k]; !named {
			merged[k] = val
		}
	}
	return json.Marshal(merged)
}

// Validate reports the first constraint violation, or nil.
func (v *UnitPriceReference) Validate() error {
	if v.present != nil {
		if !v.present["display_text"] {
			return errors.New("display_text: required property is missing")
		}
		if !v.present["unit"] {
			return errors.New("unit: required property is missing")
		}
		if !v.present["value"] {
			return errors.New("value: required property is missing")
		}
	}
	if v.Scale != nil && *v.Scale > 15 {
		return errors.New("scale: above maximum 15")
	}
	if v.Scale != nil && *v.Scale < 0 {
		return errors.New("scale: below minimum 0")
	}
	if v.Value > 9007199254740991 {
		return errors.New("value: above maximum 9007199254740991")
	}
	if v.Value < 1 {
		return errors.New("value: below minimum 1")
	}
	if v.Unit == "C62" {
		if v.Scale != nil && *v.Scale != 0 {
			return errors.New("scale: must be 0")
		}
	}
	return nil
}
